...
model_id= "/shared/hdd/nuochen/models/WhizReviewer-ML-sft_merge"#“/disk1/nuochen/models/WhizReviewer-ML-sft"#"/ssd1/models/WhizReviewer-ML-Llama3.1-8B"#"/ssd1/models/Llama-3.1-8B-Instruct"
...
# 5. Meta Review: Provide overall assessment and recommendation (Accept/Reject).
...
# Output format ONLY:
# ## Reviewer\n\n### 1. Soundness: [score]\n2. Presentation: [score]\n3. Contribution: [score]\n4. Rating: [score]\n5. Meta Review: Accept or Reject

# Replace [score] with actual numeric values. For the Meta Review, output either "Accept" or "Reject" based on your evaluation. Do not include any explanations, justifications, or reviews. Only the scores and the specified output format are required.
...
* Compared with the state-of-the-art GPU implementation, it delivers up to 10.62\(\times\) performance improvement. Furthermore, we demonstrate its effectiveness on two real world applications with delivering up to 9.15\(\times\) speedup on throughput and 26.63x power efficiency improvement on two applications (\(\pi\) estimation and Monte Carlo option pricing).
...
**Algorithms** & \multicolumn{2}{c}{**BigCrush**} & \multicolumn{2}{c}{**Throughput**} & **ThundeRiNG's** \\ (**cuRAND**) & & & & & \\ \cline{4-5} \cline{7-6}  & & & & & \\ \hline Philox4-4x2 (Xu et al., 2019) & Pass & 61.6234 & 1.9719 & **10.62x** \\ MT19937 (Xu et al., 2019) & Pass & 51.7373 & 1.6556 & 12.65\(\times\) \\ MRG32k3a (Wang et al., 2019) & 1 failure & 26.2662 & 0.8405 & **24.92x** \\ xorow (Wang et al., 2019) & 1 failure & 56.6053 & 1.8114 & 11.56\(\times\) \\ MTGP32 (Wang et al., 2019) & 1 failure & 29.1273 & 0.9321 & 22.47\(\times\) \\ \hline \hline \end{tabular}
\end{table}
...
# # 修改这里，从字典中提取 'content' 字段的字符串
# review_output = outputs[0]["generated_text"][-1]["content"]
# pattern = r"### 1\. Soundness: (\d+)\n2\. Presentation: (\d+)\n3\. Contribution: (\d+)\n4\. Rating: (\d+)\n5\. Meta Review: (Accept|Reject)"

# match = re.search(pattern, review_output)
# if match:
#     scores = {
#         "Soundness": int(match.group(1)),
#         "Presentation": int(match.group(2)),
#         "Contribution": int(match.group(3)),
#         "Rating": int(match.group(4)),
#         "Meta Review": match.group(5)
#     }
#     print(scores)
# else:
#     print("未能提取到分数信息。")